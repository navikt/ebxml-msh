...
public class EBMSReceiverListSender extends MessageSender {
     ...
     */
    public EBMSReceiverListSender(EBMSMessageData m
    ...
    public void onStart() {
        ...
        log.info("EBMS Recevier List Client init at " + new Date().toString());
        log.info("----------------------------------------------------");
    ...
    public void onResponse() throws Exception {
		...
		for(int i = messageIds.length -1; i >=  0; i--){
			...
			if (this.log != null)
				log.info("Message Id: " + messageIds[i]);					
		}*/
    ...
    public static void main(String[] args) {
        try {
            if (args.length < 2) {
                System.out.println("Usage: ebms-recvlist [partnership-xml] [config-xml]");
                System.out.println();
            ...
            EBMSReceiverListSender sender = new EBMSReceiverListSender(emd, ps);
            // Send the message.
