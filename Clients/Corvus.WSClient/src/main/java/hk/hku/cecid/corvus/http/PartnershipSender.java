...
public abstract class PartnershipSender extends HttpSender implements PartnershipOp {
     ...
     */
    protected PartnershipSender(KVPairData d) {
        super(d);
    }
    ...
    protected void onStart() {
        ...
        log.info("Partnership HTTP Client init at " + new Date().toString());
        log.info("");
        log.info("Sending Partnership HTTP Request with following configuration");
        log.info("------------------------------------------------------------------");
        log.info("Partnership Operation : " + this.getPartnershipOperationMapping().get(new Integer(pOp)));
        if (data != null)
     ...
     *
     * @throws Exception When fail to verify by PartnershipOpVerifer.
     */
