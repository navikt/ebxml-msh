	            ...
	            WebServicesResponse response) throws SOAPRequestException
	 {
	        ...
	        {	        		        		        
	        	...
	        	for (int i = 0; i < AS2_MODS_NAME.length; i++){
		        	...
		        	AS2Processor.core.log.info("test");
		        	
		        	if (as2ModMaxThread[i] != -1 && as2ModEI[i] != -1){
		        		as2ModFlag[i] 		= true;			        		
		        		AS2Processor.core.log.info("thread" + as2ModMaxThread[i]);
		        		AS2Processor.core.log.info("ei" + as2ModEI[i]);
		        	}		        	
	        	}
	        		        		        		        		        	               		        		        		    				       
		        if (activeModuleStatus)
		        	AS2Processor.getModuleGroup().startActiveModules();		        	
		        else
		        	AS2Processor.getModuleGroup().stopActiveModules();
		        
		        AS2Processor.core.log.info("AS2 Active Modules new status: " + String.valueOf(activeModuleStatus));
		        ...
		        for (int i = 0; i < AS2_MODS_NAME.length; i++)
		        {
		        	...
		        	if (as2ModFlag[i]){
			        	am = (ActiveTaskModule) (AS2Processor.getModuleGroup().getModule(AS2_MODS_NAME[i]));
			        	if (am != null && am.getMonitor() != null)
			        	{
			        		am.getMonitor().setMaxThreadCount(as2ModMaxThread[i]);	
			        		am.setExecutionInterval(as2ModEI[i]);
			        		AS2Processor.core.log.info(AS2_MODS_NAME[i] 
			        		                          +" Thread set to: " 
	        ...
	        {
	        	AS2Processor.core.log.debug("Configuration Error", e);
	        	generateReply(response
	        				 ,"Fail"
